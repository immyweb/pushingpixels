...
export default class About {
	...
	mainTl() {
		mainTl
			...
			.add('ellipsesIn')
			.fromTo(this.ellipseBkgndTop, 0.5,
				{ scale: 0, transformOrigin: 'center center' },
				{ scale: 1, autoAlpha: 0.5, ease: Power4.easeInOut })
			.fromTo(this.ellipseBkgndBottom, 0.5,
				{ scale: 0, transformOrigin: 'center center' },
				{ scale: 1, autoAlpha: 1, ease: Power4.easeInOut }, '-=0.25')

			.add('panelsIn')
			.fromTo(this.rectLight, 0.5,
				{ scaleX: 0, transformOrigin: 'center center' },
				{ scaleX: 1, autoAlpha: 1, ease: Power4.easeInOut })
			.fromTo(this.rectMid, 0.5,
				{ scaleX: 0, transformOrigin: 'center center' },
				{ scaleX: 1, autoAlpha: 1, ease: Power4.easeInOut }, '-=0.25')
			.fromTo(this.content, 0.75,
				{ scaleY: 0, transformOrigin: 'center top' },
				{ scaleY: 1, autoAlpha: 1, ease: Power4.easeInOut }, '-=0.25')
			.to(this.contentHolder, 0.75, { autoAlpha: 1, ease: Power4.easeInOut }, '-=0.25')

			.add('linesIn')
			.staggerFromTo(this.linesLeft, 0.5,
				{ scaleY: 0, transformOrigin: 'center top' },
				{ scaleY: 1, autoAlpha: 1, ease: Elastic.easeOut.config(1.75, 0.3) }, 0.02, 'panelsIn')
			.staggerFromTo(this.linesBottom, 0.5,
				{ scaleY: 0, transformOrigin: 'center top' },
				{ scaleY: 1, autoAlpha: 1, ease: Elastic.easeOut.config(1.75, 0.3) }, 0.02, 'panelsIn')

			.add('dotsIn')
			.staggerFromTo(this.dotsSmall, 0.5,
				{ scale: 0, transformOrigin: 'center top' },
				{ scale: 1, autoAlpha: 1, ease: Power4.easeInOut }, 0.02, 'panelsIn+=0.5')
			.staggerFromTo(this.dotsLarge, 0.4,
				{ scale: 0, transformOrigin: 'center top' },
				{ scale: 1, autoAlpha: 1, ease: Power4.easeInOut }, 0.02, 'panelsIn+=0.5')
		;
